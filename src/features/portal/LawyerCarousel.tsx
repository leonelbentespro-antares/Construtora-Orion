...
const HireModal: React.FC<{ lawyer: Lawyer; onClose: () => void }> = ({ lawyer, onClose }) => {
  ...
  const responseH = lawyer.avg_response_hours
    ? lawyer.avg_response_hours < 1
      ? '< 1h'
      : `${Math.round(lawyer.avg_response_hours)}h`
    : '—'
  ...
  return (
    <AnimatePresence>
      ...
      >
        {/* Header gradient */}
        <div className="bg-gradient-to-br from-[#1D4ED8] to-[#6366F1] px-6 pt-6 pb-8 text-white text-center relative shrink-0">
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-4 border-t border-[#E5E5EA] bg-white shrink-0 space-y-2">
          ...
          <p className="text-[11px] text-center text-[#86868B]">
            Pagamento seguro via Stripe. Sem mensalidade, pague apenas quando contratar.
          </p>
        </div>
      </motion.div>
    </AnimatePresence>
...
const LawyerCard: React.FC<{ lawyer: Lawyer; onClick: () => void }> = ({ lawyer, onClick }) => {
  ...
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.98 }}
      onClick={onClick}
      className="flex-none w-[200px] bg-white border border-[#E5E5EA] rounded-[16px] p-4 cursor-pointer hover:border-[#2563EB]/40 transition-colors"
    >
      {/* Avatar */}
      ...
      <div className="mt-3 w-full py-1.5 text-center text-xs font-medium text-[#2563EB] bg-[#EFF6FF] rounded-[8px]">
        Ver perfil →
      </div>
    </motion.div>
  )
...
export const LawyerCarousel: React.FC = () => {
  ...
  const { data: lawyers = [], isLoading } = useLawyerSearch({
    priorityMode: isCompany,
  })
